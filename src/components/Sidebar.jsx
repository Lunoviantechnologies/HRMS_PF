...
const Sidebar = () => {
  ...
  const collapsibleSection = (id, icon, title, children) => (
    <>
      <li className="list-group-item text-white border-0" style={{ backgroundColor: "rgb(2,2,95)" }}>
        <button className="btn border-0 text-white w-100 text-start" onClick={() => toggleSection(id)} >
  ...
  const SidebarContent = () => (
    <div
      ...
      style={{ width: "250px", maxHeight: "calc(100vh - 56px)", overflowY: "auto", backgroundColor: "rgb(2,2,95)" }}
    >
      <ul className="list-group list-group-flush" style={{ paddingBottom: "100px" }}>
        {collapsibleSection("employeeMgmt", "bi-people", "Employee Management", <>
          {navItem("Add Employee", "/dashboard/addEmployee")}
          {navItem("All Employees", "/dashboard/allEmployees")}
        </>)}

        {collapsibleSection("attendanceMgmt", "bi-calendar-check", "Attendance Management", <>
        ...
        {collapsibleSection("leaveMgmt", "bi-envelope-open", "Leave Management", <>
          {navItem("Leave Requests", "/dashboard/leaveRequest")}
          {navItem("Leave Policy", "/dashboard/leavePolicy")}
        </>)}

        {collapsibleSection("payrollMgmt", "bi-cash-stack", "Payroll Management", <>
        ...
        {collapsibleSection("reportsAnalytics", "bi-bar-chart", "Reports & Analytics", <>
        ...
        {collapsibleSection("systemSettings", "bi-gear-wide-connected", "System Settings", <>
